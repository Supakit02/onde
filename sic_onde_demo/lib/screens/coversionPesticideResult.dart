...
class ConversionPesticideResult extends StatefulWidget {
  const ConversionPesticideResult({Key? key}) : super(key: key);

  @override
  _ConversionPesticideResultState createState() =>
      _ConversionPesticideResultState();
}

class _ConversionPesticideResultState extends State<ConversionPesticideResult> {
  ...
  Future<void> _onDoneCallback(IoData data) async {
    ...
    Get.find<APIController>().setSecondRawResult(Manager.result.result);
    ...
    if (serviceController.lastEnzyme) {
      ...
      try {
        ...
        for (final _p in Manager.result.peak) {
          _p.concentration = _concentration; //! Be care
          _p.m = Get.find<SettingConstantLinear>().m;
          _p.c = Get.find<SettingConstantLinear>().c;
        }
    ...
    Get.find<APIController>().setResult(_result);
